...
pub async fn create_routing_rule(
    ...
    routing_service::create(state.inner(), payload).await
}
...
pub async fn update_routing_rule(
    ...
    routing_service::update(state.inner(), id, payload).await
}

#[tauri::command]
pub async fn delete_routing_rule(state: State<'_, AppState>, id: String) -> Result<bool, String> {
    routing_service::delete(state.inner(), id).await
}